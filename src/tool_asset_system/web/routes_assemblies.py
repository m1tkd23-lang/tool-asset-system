...
def assemblies_new():
    ...
    New Assembly:
      - parts を検索して選択
      - ASMを作り、選択したpartsを一括で assembly_items に入れる
      - itemsから signature を作って display_name に反映（display_name未入力時）
    """
    # parts検索用（GET/POSTどちらでも使う）
    ...
    if request.method == "POST":
        ...
        selected_parts = request.form.getlist("selected_parts")  # asset_code の配列
        # role/qty は dictで受ける
        # role_{asset_code}, qty_{asset_code}
        ...
        try:
            code = add_assembly(
                display_name=display_name,  # Noneなら仮名になる（services側）
            ...
            flash(f"Added: {code}", "ok")
            return redirect(url_for("assemblies.assembly_detail", assembly_code=code))
